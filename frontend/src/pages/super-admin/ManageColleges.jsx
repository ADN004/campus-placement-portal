      ...
      <AnimatedSection delay={0.16}>
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="overflow-x-auto">
            {/*
              Declared proportions. Six columns of short values left to a
              browser's own sharing gave the Actions column 267px to hold a 42px
              control, stranding it 166px from the status it sits beside — the
              same fault the officer's PRN table and the student's applications
              table had. Status is right-aligned so the two travel together.
            */}
                ...
                {filteredColleges.map((college) => (
                  <tr key={college.id} className={`hover:bg-gray-50 ${!college.is_active ? 'opacity-60' : ''}`}>
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm font-medium text-gray-900 truncate">{college.college_name}</div>
