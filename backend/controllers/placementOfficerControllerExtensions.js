...
export const manuallyAddStudentToJob = async (req, res) => {
  ...
  try {
    ...
    // Verify job exists and belongs to officer's college
    ...
    if (jobCheck.rows.length === 0) {
      ...
      return res.status(404).json({
        success: false,
        message: 'Job not found in your college',
      });
    }
    ...
    const reviewNotes = `[MANUAL ADDITION by Placement Officer on ${new Date().toISOString()}]\n${notes}`;
